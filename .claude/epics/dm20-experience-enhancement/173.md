...
<button id="mic-btn" class="voice-control" title="Speak your action">
    ...
    <span class="listening-indicator" hidden>●</span>
</button>
```
...
The WebSocket handler already processes `player_action` messages. Voice-transcribed text arrives in the same format — no server changes needed beyond accepting the optional `source: "voice"` field for logging.
...
- `src/dm20_protocol/party/static/` — Player UI: mic button, STT JS, listening indicator CSS
- `tests/` — UI integration tests (if applicable)

## Dependencies

- [ ] **Task #172 (Voice Registry + Audio Streaming)** must be completed — establishes audio infrastructure
...
- [ ] Graceful fallback verified (STT unavailable → text input works)
