...
Remove in-progress label, add completion comment, and close:
```bash
# Remove in-progress label (ignore error if label doesn't exist)
gh issue edit $ARGUMENTS --remove-label "in-progress" 2>/dev/null || true
